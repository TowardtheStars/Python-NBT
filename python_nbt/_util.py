...
class RestrictedList(list):
    ...
    def extend(self, iterable, trim=False):
        if not trim:
            ...
            if not all(cond):
                raise ValueError("Contains value that cannot be put into this RestricList")
        else:
